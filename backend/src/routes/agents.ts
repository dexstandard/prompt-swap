...
const baseSelect =
  'SELECT id, user_id, model, status, created_at, name, token_a, token_b, target_allocation, min_a_allocation, min_b_allocation, risk, review_interval, agent_instructions FROM agents';
...
export default async function agentRoutes(app: FastifyInstance) {
  ...
  app.post('/agents', async (req, reply) => {
    ...
    const existing = db
      .prepare('SELECT id FROM agents WHERE user_id = ? AND token_a = ? AND token_b = ?')
      .get(body.userId, tokenA, tokenB) as { id: string } | undefined;
    if (existing)
      return reply
        .code(400)
